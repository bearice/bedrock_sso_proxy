...
use bedrock_sso_proxy::database::entities::{ModelCost, UserRecord, ApiKeyRecord};
use chrono::Utc;
...
async fn test_redis_cache_complex_decimal_operations() {
    ...
    for (test_name, decimal_value) in test_cases {
        ...
        typed_cache
            ...
            .await
            .unwrap_or_else(|_| panic!("Failed to serialize ModelCost with decimal: {}", decimal_value));
        ...
        // Cleanup
        typed_cache.delete(&format!("decimal_test_{}", test_name)).await.unwrap();
    }
}
...
async fn test_redis_health_check() {
    ...
    assert!(matches!(result.status, bedrock_sso_proxy::health::HealthStatus::Healthy));
...
async fn test_redis_complex_model_cost_decimals() {
    ...
    typed_cache.set("complex_test", &test_model_cost).await.unwrap();
    let cached = typed_cache.get("complex_test").await.unwrap().unwrap();
    
    assert_eq!(cached.id, test_model_cost.id);
    assert_eq!(cached.input_cost_per_1k_tokens, test_model_cost.input_cost_per_1k_tokens);
    assert_eq!(cached.output_cost_per_1k_tokens, test_model_cost.output_cost_per_1k_tokens);
    assert_eq!(cached.cache_write_cost_per_1k_tokens, test_model_cost.cache_write_cost_per_1k_tokens);
    assert_eq!(cached.cache_read_cost_per_1k_tokens, test_model_cost.cache_read_cost_per_1k_tokens);
