...
mod tests {
    ...
    fn test_new_regional_mapping() {
        ...
        assert_eq!(mapping.get_prefix("ap-southeast-1"), Some("apac".to_string()));
    ...
    fn test_add_mapping() {
        ...
        assert_eq!(mapping.get_prefix("custom-region"), Some("custom".to_string()));
    }
    ...
    fn test_add_regional_prefix() {
        ...
        let result = mapping.add_regional_prefix("anthropic.claude-sonnet-4-20250514-v1:0", "us-east-1");
        assert_eq!(result, "us.anthropic.claude-sonnet-4-20250514-v1:0");
        
        // Test AP region gets apac prefix
        let result = mapping.add_regional_prefix("anthropic.claude-sonnet-4-20250514-v1:0", "ap-southeast-1");
        assert_eq!(result, "apac.anthropic.claude-sonnet-4-20250514-v1:0");
        
        // Test model that already has prefix - should not double-prefix
        let result = mapping.add_regional_prefix("us.anthropic.claude-sonnet-4-20250514-v1:0", "us-east-1");
        assert_eq!(result, "us.anthropic.claude-sonnet-4-20250514-v1:0");
        
        // Test unknown region - should return original
        let result = mapping.add_regional_prefix("anthropic.claude-sonnet-4-20250514-v1:0", "unknown-region");
    ...
    fn test_strip_regional_prefix() {
        ...
        let result = mapping.strip_regional_prefix("unknown.anthropic.claude-sonnet-4-20250514-v1:0");
    ...
    fn test_parse_region_from_model_id() {
        ...
        let region = mapping.parse_region_from_model_id("us.anthropic.claude-sonnet-4-20250514-v1:0");
        assert_eq!(region, Some("us-east-1".to_string()));
        
        let region = mapping.parse_region_from_model_id("apac.anthropic.claude-sonnet-4-20250514-v1:0");
        assert_eq!(region, Some("ap-southeast-1".to_string()));
        
        let region = mapping.parse_region_from_model_id("eu.anthropic.claude-sonnet-4-20250514-v1:0");
        ...
        let region = mapping.parse_region_from_model_id("unknown.anthropic.claude-sonnet-4-20250514-v1:0");
        assert!(region.is_none());
    }
    ...
    fn test_configurable_prefix_mapping() {
        ...
        assert_eq!(mapping.get_region_for_prefix("us"), Some("us-east-1".to_string()));
        assert_eq!(mapping.get_region_for_prefix("apac"), Some("ap-southeast-1".to_string()));
        assert_eq!(mapping.get_region_for_prefix("eu"), Some("eu-west-1".to_string()));
        
        // Test configurable mapping
        mapping.add_prefix_mapping("us".to_string(), "us-west-2".to_string());
        assert_eq!(mapping.get_region_for_prefix("us"), Some("us-west-2".to_string()));
        
        // Test custom prefix
        mapping.add_prefix_mapping("custom".to_string(), "custom-region-1".to_string());
        assert_eq!(mapping.get_region_for_prefix("custom"), Some("custom-region-1".to_string()));
    }
